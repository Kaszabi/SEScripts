...
public void Main(string argument, UpdateType updateSource) {
    IMyTextPanel display = GridTerminalSystem.GetBlockWithName("BIG 3D - Screen") as IMyTextPanel;
    String[] lcdLines = new String[20];
    IMyPistonBase pistonX = GridTerminalSystem.GetBlockWithName("BIG 3D - X axis") as IMyPistonBase;
    IMyPistonBase pistonY = GridTerminalSystem.GetBlockWithName("BIG 3D - Y axis") as IMyPistonBase;
    IMyPistonBase pistonZ = GridTerminalSystem.GetBlockWithName("BIG 3D - Z axis") as IMyPistonBase;
    IMyShipWelder welder = GridTerminalSystem.GetBlockWithName("BIG 3D - Welder") as IMyShipWelder;

    IMyInteriorLight btnOrigo = GridTerminalSystem.GetBlockWithName("BIG 3D - Going Origo") as IMyInteriorLight;
    IMyInteriorLight btnStart = GridTerminalSystem.GetBlockWithName("BIG 3D - Start") as IMyInteriorLight;
    IMyInteriorLight btnStop = GridTerminalSystem.GetBlockWithName("BIG 3D - Stop") as IMyInteriorLight;
    IMyInteriorLight btnWelder = GridTerminalSystem.GetBlockWithName("BIG 3D - Welder Active") as IMyInteriorLight;
    ...
    lcdLines[0] = $"Printer Coords (X, Y, Z): {curX.ToString("0.00")} {curY.ToString("0.00")} {curZ.ToString("0.00")}";
    lcdLines[1] = $"Current step: {step}";
    lcdLines[2] = $"Welding Status: {welder.Enabled}";
